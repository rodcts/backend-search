...
def _get_ml_url(product_name: str, estado: str) -> str:
    ...
    product_url_slug = product_name.lower().replace(' ', '-').strip()
...
async def _scrape_mercado_livre(client: httpx.AsyncClient, product_name: str, estado: str):
    ...
    log.info(f"[Scraper] Buscando Mercado Livre para: {product_name} (Estado: {estado})")
    ...
    try:
        ...
        if not price_elements:
            log.warning(f"[Scraper] Nenhum preço encontrado no ML para: {product_name} (Estado: {estado})")
            return None
        ...
        log.error(f"[Scraper] Falha CRÍTICA ao raspar ML para {product_name} (Estado: {estado})", exc_info=True)
        return {"site": "mercadolivre", "error": f"failed_to_scrape: {e}"}
    
    return None


async def scrape_sites_in_parallel(product_name: str, estado: str) -> (list | None, list):
    """ 
