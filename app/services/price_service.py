...
from app.repositories import analytics_repo, cache_repo
    ...
    if await cache_repo.acquire_lock(product_cache_key):
        ...
        try:
            ...
            if prices:
                ...
                await cache_repo.set_price_in_cache(product_cache_key, stats)
                # Salva no BD Analítico (usando a chave de cache)
                await analytics_repo.save_analytic_data(
                    product_cache_key, stats, raw_data
                )
            ...
            await cache_repo.release_lock(product_cache_key)
    else:
...
async def _wait_for_leader_to_finish(
    product_cache_key: str, timeout_seconds: int = 20
) -> dict | None:
    """
